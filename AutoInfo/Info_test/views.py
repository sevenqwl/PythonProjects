...
from Info_test.utils import acs_script, get_excel
import pytz
import yaml
        ...
        if multilist:
            ...
            if searchtype=="search":
                ...
                if search_keywords is not None:
                    parameters_list = ["_lastInform", "Device.LAN.IPAddress","Device.LAN.MACAddress", "Device.DeviceInfo.ProductClass", "VirtualParameters.SIP1", "VirtualParameters.SIP1 Status", "VirtualParameters.SIP1 RegistrarServer", "VirtualParameters.SIP2", "VirtualParameters.SIP1 Status", "VirtualParameters.SIP2 RegistrarServer"]
                    for i in parameters_list:
                ...
                try:
                    ...
                    if IP:
                        ...
                        if request_url:
                            ...
                            logcontent = acs_script.request_logcontent(reqeust_obj_stdout, IP)  # 返回执行结果匹配
                        else:
...
def logfile(request):
    logfile = "media/%s" %(request.GET.get('logfile'))
    print(logfile)
...
def excel_export(request):
    ...
    genieacs_list = collection.find(search_conditions, {"Device.LAN.IPAddress": 1, "Device.LAN.MACAddress": 1,
                                                        "Device.DeviceInfo.ProductClass": 1, "VirtualParameters": 1,
                                                        "_lastInform": 1})
    total_count = genieacs_list.count()
    ...
    parameters_list = ["Device.LAN.IPAddress", "VirtualParameters.SIP1", "VirtualParameters.SIP1 Status",  "VirtualParameters.SIP1 RegistrarServer",  "VirtualParameters.SIP1 OutboundProxy", "VirtualParameters.SIP2", "VirtualParameters.SIP2 Status",  "VirtualParameters.SIP2 RegistrarServer",  "VirtualParameters.SIP2 OutboundProxy", ]

    for item in enumerate(genieacs_list):
        # print(item[0], item[1])
        ws.write(item[0], 0, item[1].get('_id'))
        for i in enumerate(parameters_list):
            ws.write(item[0], i[0]+1, get_excel.get_excel_value(i[1], item[1]))
            print(item[0], i[0]+1, get_excel.get_excel_value(i[1], item[1]))
        # ws.write(item[0], 1, item[1].get('Device').get('DeviceInfo').get('ProductClass').get('_value'))
        # ws.write(item[0], 2, item[1].get('Device').get('LAN').get('IPAddress').get('_value'))
        # ws.write(item[0], 3, item[1].get('Device').get('LAN').get('MACAddress').get('_value'))
        # ws.write(item[0], 4, item[1]['VirtualParameters']['SIP2']['_value'])
        # ws.write(item[0], 5, item[1].get('Device').get('VirtualParameters').get('SIP2').get('_value'))
    ...
    response['Content-Disposition'] = 'attachment; filename=分机信息.xlsx'
