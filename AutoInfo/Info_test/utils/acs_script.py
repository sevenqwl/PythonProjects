...
def get_device_info(IP):
    query_str = '{"Device.LAN.IPAddress":"' + IP + '"}'
...
def check_extension(extension):  # 检测是否存在重复分机号,1小时内的分机号
    if extension:
        ...
        yes_time = now_time + datetime.timedelta(hours=-1)
...
def get_request_url(request_data_dict, sipinfo_dict):
    ...
    if "#" in IP:
        ...
        if device_id and device_manufacturer:
            ...
            extension = sipinfo_dict.get('Sip1_AuthUserName')
            ...
            request_url = None
            error_code = error_code
    request_url_dict = {'request_url': request_url, 'error_code': error_code}
...
def request_logcontent(reqeust_obj_stdout, IP):
    if "HTTP/1.1 200 OK" in reqeust_obj_stdout:
        ...
        print("%s   HTTP/1.1 202 Task faulted" % IP)
        logcontent = "%s   HTTP/1.1 202 Task faulted" % IP
    else:
        print("%s  配置失败啦......" % IP)
        logcontent = "%s    %s" %(IP, reqeust_obj_stdout)
