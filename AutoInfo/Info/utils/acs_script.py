...
# SIP2 参数
    ...
    "UpgradeType": {  # 配置文件升级类型 TFTP HTTP HTTPS
        "parameter": "",
        "value": "HTTP",
        "type": "xsd:string",
    },
...
class Genieacs_api(object):
    ...
    def setParameterValues(self, device_id, parameter_info):
        ...
        request_api = "'%s/devices/%s/tasks?timeout=10000&connection_request' -X POST --data '%s'" % (self.Genieacs_http_url, device_id, parameter_str)
        return request_api
...
def sipinfo_method_dict(request_dict, sipinfo_dict, method):
    if method == "request_yaml_api":
        ...
        sipinfo_dict = None
    # print('sipinfo_dict: ', sipinfo_dict)
    return sipinfo_dict
...
def get_request_url(request_data_dict, sipinfo_dict):
    ...
    Sip1_AuthUserName = sipinfo_dict.get('Sip1_AuthUserName')
    extension = Sip1_AuthUserName.strip() if Sip1_AuthUserName is not None else ""
    if "#" in IP:
        request_url = None
        error_code = 22
        print("%s    注释了......." %IP)
        ...
        if device_id and device_manufacturer:
            ...
            try:
                for info in enumerate(sipinfo_dict):
                    if info[1] != "IP":
                        parameter, value = SIP_INFO[device_manufacturer][info[1]]['parameter'], SIP_INFO[device_manufacturer][info[1]]['type']
                        if parameter:
                            parameter_info += str_format(parameter, sipinfo_dict[info[1]], value) + ', '
            except:
                error_code = 30
                parameter_info = ""
