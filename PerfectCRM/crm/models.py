...
class ClassList(models.Model):
    ...
    semster = models.PositiveSmallIntegerField(verbose_name="学期")
    teachers = models.ForeignKey("UserProfile")
    start_date = models.DateField(verbose_name="开班日期")
    end_date = models.DateField(verbose_name="结业日期", blank=True, null=True)

    def __str__(self):
        return "%s %s %s" %(self.branch, self.course, self.semster)

    class Meta:
        unique_together = ('branch', 'course', 'semster')  # 联合唯一
        verbose_name_plural = "班级表"


class CourseRecord(models.Model):
    '''上课记录'''
    from_course = models.ForeignKey("ClassList", verbose_name="班级")
    ...
    def __str__(self):
        return "%s %s" %(self.from_course, self.day_num)

    class Meta:
        unique_together = ('from_course', 'day_num')  # 联合唯一
