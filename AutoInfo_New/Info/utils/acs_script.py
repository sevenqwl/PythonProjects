...
# 拼接成请求的api，自定义模版配置
def request_configd_api(set_dict):
    Parameter_info = ""
    # set_dict = {'IP': '10.6.27.189', '分机号': '8489', '电话平台': '扬州行政电话'}
    IP = set_dict.get('IP')
    extension = set_dict.get('分机号')
    platform = set_dict.get('电话平台')
    if "#" in IP:
        IP = ""
    if IP:
        device_info = get_device_info(IP)
        if device_info['device_id'] and device_info['device_manufacturer']:
            ...
            configuration_options = models.PhoneConfigurationOptions.objects.get(platform=platfrom)
            if configuration_options:

                try:
                    Parameter_info += str_format(SIP_INFO[device_manufacturer]['Sip1_DisplayName']['parameter'], set_dict['extension'], SIP_INFO[device_manufacturer]['Sip1_DisplayName']['type']) + ', '
                    Parameter_info += str_format(SIP_INFO[device_manufacturer]['Sip1_Label']['parameter'], set_dict['extension'], SIP_INFO[device_manufacturer]['Sip1_Label']['type']) + ', '
                    Parameter_info += str_format(SIP_INFO[device_manufacturer]['Sip1_UserName']['parameter'], set_dict['extension'], SIP_INFO[device_manufacturer]['Sip1_UserName']['type']) + ', '
                    Parameter_info += str_format(SIP_INFO[device_manufacturer]['Sip1_AuthUserName']['parameter'], set_dict['extension'], SIP_INFO[device_manufacturer]['Sip1_AuthUserName']['type']) + ', '
                    Parameter_info += str_format(SIP_INFO[device_manufacturer]['Sip1_AuthPassword']['parameter'], set_dict[configuration_options.Sip1_AuthPassword], SIP_INFO[device_manufacturer]['Sip1_AuthPassword']['type']) + ', '
                    Parameter_info += str_format(SIP_INFO[device_manufacturer]['Sip1_RegistrarServer']['parameter'], set_dict[configuration_options.Sip1_RegistrarServer], SIP_INFO[device_manufacturer]['Sip1_RegistrarServer']['type']) + ', '
                    Parameter_info += str_format(SIP_INFO[device_manufacturer]['Sip1_UseOutboundProxy']['parameter'], set_dict[configuration_options.Sip1_UseOutboundProxy], SIP_INFO[device_manufacturer]['Sip1_UseOutboundProxy']['type']) + ', '
                    Parameter_info += str_format(SIP_INFO[device_manufacturer]['Sip1_OutboundProxy']['parameter'], set_dict[configuration_options.Sip1_OutboundProxy], SIP_INFO[device_manufacturer]['Sip1_OutboundProxy']['type']) + ', '
                except:
                    pass
                print(Parameter_info)
                Parameter_info = Parameter_info.rsplit(', ', 1)[0]
