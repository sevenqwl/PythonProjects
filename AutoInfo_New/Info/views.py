...
def config_phone(request):
    ...
    # request_data_dict = acs_script.get_excel('media/20180712092221_8779_acs_config.xlsx')

    with open(logfile, 'w+') as file:
        for j in request_data_dict:
            print(request_data_dict[j])
            REQUEST_SET_API = acs_script.request_configd_api(request_data_dict[j])
            ...
            #         logcontent = "%s   HTTP/1.1 200 OK" % request_data_dict[j]['IP']
            ...
            #         logcontent ="%s   HTTP/1.1 202 Task queued but not processed" % request_data_dict[j]['IP']
            #     else:
            #         print(REQUEST_OBJ_STDOUT)
            #         logcontent = REQUEST_OBJ_STDOUT
            # else:
            #     print("%s   REQUEST_SET_API is None" % request_data_dict[j]['IP'])
            #     logcontent = "%s   REQUEST_SET_API is None" % request_data_dict[j]['IP']
            # file.write(logcontent + '\n')

            # 写入数据库
            # models.PhoneinfoSettingRecord.objects.create(date='2018-07-16',time='11:00:00', floor='10F', ip='10.6.27.189', extension='8002', settingip='null', ping_status='online', exec_type='1', exec_status='1', exec_remarks='1', exec_logs='xxx')
        else:
    ...
    return HttpResponse(logallcontent)
